...
describe('Menu Data Validation', () => {
  ...
  describe('Full Menu Quality', () => {
    ...
    it('should find coffee-related items', async () => {
      ...
      if (menuData.items.length > 0) {
        ...
        const coffeeItems = menuData.items.filter(item => 
        ...
        fail('No menu items found - menu scraping may be broken');
      }
    }, 30000);
  });
  ...
  describe('Menu Categories Quality', () => {
    it('should return meaningful categories', async () => {
      ...
      const hasSpecificCategories = categoriesData.categories.some(category => 
    ...
    it('should find items when searching for coffee', async () => {
      ...
      if (searchData.resultsFound === 0) {
        ...
        if (fullMenuData.totalItems > 0) {
          console.warn('Menu has items but coffee search returned no results - search may need improvement');
        } else {
          fail('No menu items found at all - menu scraping is broken');
        }
      } else {
        ...
        const relevantResults = searchData.items.filter(item =>
          item.name.toLowerCase().includes('coffee') ||
